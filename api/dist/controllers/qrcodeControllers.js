...
class QRCodeController {
    constructor() {
         ...
         */
        this.generateTOTPQRCode = async (req, res) => {
            try {
                ...
                const result = await totpService_1.totpService.generateTOTPSecret(user.id, user.email);
                if (result.success) {
                    res.status(200).json({
                        success: true,
                        data: {
                            ...
                            testing: {
                                currentToken: totpService_1.totpService.getCurrentTOTPToken(result.secret),
         ...
         */
        this.getCurrentTOTPQRCode = async (req, res) => {
            try {
                const errors = (0, express_validator_1.validationResult)(req);
                if (!errors.isEmpty()) {
                    res.status(400).json({
                ...
                res.status(200).json({
                    ...
                    data: {
                        ...
                        testing: {
                            currentToken: totpService_1.totpService.getCurrentTOTPToken(user.mfaSecret),
                            timeRemaining: totpService_1.totpService.getTimeRemaining()
                        }
                    }
                });
         ...
         */
        this.verifyAndEnableTOTP = async (req, res) => {
            try {
                const errors = (0, express_validator_1.validationResult)(req);
                if (!errors.isEmpty()) {
                    res.status(400).json({
                ...
                // Verify TOTP token
                const verification = await totpService_1.totpService.verifyTOTPSetup(secret, token);
                if (verification.success && verification.valid) {
                    // Enable TOTP for user
                    const result = await totpService_1.totpService.enableTOTP(user.id, secret, token);
                    if (result.success) {
                        res.status(200).json({
                            success: true,
         ...
         */
        this.testTOTPToken = async (req, res) => {
            try {
