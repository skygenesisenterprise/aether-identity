...
exports.PermissionHelper = exports.AVAILABLE_APPLICATIONS = exports.AVAILABLE_SERVICES = exports.PERMISSIONS = exports.ROLES = void 0;
...
exports.AVAILABLE_APPLICATIONS = [
    {
        id: 'admin-console',
        name: 'Admin Console',
...
class PermissionHelper {
    ...
    static isValidRole(role) {
        return Object.keys(exports.ROLES).includes(role);
    }
    // Obtenir la configuration d'un rôle
    static getRoleConfig(role) {
        return exports.ROLES[role];
    }
    // Obtenir toutes les permissions d'un rôle
    static getRolePermissions(role) {
        const roleConfig = exports.ROLES[role];
        if (!roleConfig)
            return [];
    ...
    static isValidPermission(permission) {
        return Object.keys(exports.PERMISSIONS).includes(permission);
    }
    // Obtenir la configuration d'une permission
    static getPermissionConfig(permission) {
        return exports.PERMISSIONS[permission];
    }
    ...
    static getAccessibleServices(role) {
        const rolePermissions = this.getRolePermissions(role);
        return exports.AVAILABLE_SERVICES.filter(service => {
            return service.requiredPermissions.some(perm => rolePermissions.includes(perm) ||
                this.matchesWildcard(rolePermissions, perm));
        });
    }
    // Obtenir les applications accessibles pour un rôle
    static getAccessibleApplications(role) {
        const rolePermissions = this.getRolePermissions(role);
        return exports.AVAILABLE_APPLICATIONS.filter(app => {
            return app.requiredPermissions.some(perm => rolePermissions.includes(perm) ||
                this.matchesWildcard(rolePermissions, perm));
        });
    }
    // Vérifier si une permission correspond à un wildcard
    static matchesWildcard(permissions, permission) {
    ...
    static validateRoleHierarchy(currentRole, targetRole) {
