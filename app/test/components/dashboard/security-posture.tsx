  ...
  const securityItems: SecurityItem[] = [
    {
      ...
      maxValue: 100,
      status: mfaAdoptionRate >= 90 ? "good" : mfaAdoptionRate >= 70 ? "warning" : "critical",
    },
    {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className={cn("flex items-center gap-1.5 rounded-full px-2.5 py-1", 
              securityScore >= 80 ? "bg-accent/10" : securityScore >= 60 ? "bg-amber-500/10" : "bg-destructive/10"
            )}>
              {securityScore >= 80 ? (
                <ShieldCheck className={cn("h-3.5 w-3.5", getScoreColor(securityScore))} />
              ) : (
                <ShieldAlert className={cn("h-3.5 w-3.5", getScoreColor(securityScore))} />
              )}
              <span className={cn("text-sm font-semibold", getScoreColor(securityScore))}>
                {securityScore}
              </span>
            </div>
          </div>
            ...
            <div key={item.label} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={cn(
                  ...
                  item.status === "warning" ? "text-amber-400" : "text-destructive"
                )}>
                  {item.value}%
                </span>
              </div>
              <Progress 
                ...
                className={cn("h-1.5 bg-secondary", 
                  item.status === "good" ? "[&>div]:bg-accent" :
                  item.status === "warning" ? "[&>div]:bg-amber-400" : "[&>div]:bg-destructive"
                )}
              />
            </div>
          ))}
          ...
          <div className="flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 p-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-400">
                {flaggedIdentities} High-Risk {flaggedIdentities === 1 ? "Identity" : "Identities"}
              </p>
