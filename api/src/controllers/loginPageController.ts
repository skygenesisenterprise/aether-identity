...
export class LoginPageController {
   ...
   */
  public getLoginPage = async (req: Request, res: Response): Promise<void> => {
    try {
      const {
        client_id,
        redirect_uri,
      ...
      let validRedirectUri = redirect_uri as string;
      if (redirect_uri) {
      ...
      const loginPageHtml = this.generateLoginPageHtml({
        client: {
          id: client.clientId,
          name: client.name,
          logo: custom_logo as string || client.logoUrl,
        ...
        customCss: custom_css as string,
        oauthParams: {
   ...
   */
  public getLoginConfig = async (req: Request, res: Response): Promise<void> => {
    try {
      const { client_id, lang = 'fr' } = req.query as any;
   ...
   */
  public getLoginEmbed = async (req: Request, res: Response): Promise<void> => {
    try {
      const {
        client_id,
        redirect_uri,
      ...
      const embedHtml = this.generateEmbedHtml({
        ...
        theme: theme as string,
        lang: lang as string,
        dimensions: { width: width as string, height: height as string }
      });
   ...
   */
  public validateLogin = async (req: Request, res: Response): Promise<void> => {
    try {
      const { email, password, client_id, remember_me = false } = req.body as any;
   ...
   */
  private generateLoginPageHtml(params: {
    client: any;
    redirectUri: string;
    theme: string;
    lang: string;
...
<body data-theme="${theme}">
   ...
   */
  private generateEmbedHtml(params: {
    client: any;
    redirectUri: string;
    theme: string;
    lang: string;
...
<body>
   ...
   */
  private getCSP(embedded: boolean): string {
    const basePolicies = [
      "default-src 'self'",
  ...
  private sendErrorPage(
    ...
    const messages = errorMessages[lang as keyof typeof errorMessages] || errorMessages.fr;
    const message = messages[errorCode as keyof typeof messages] || errorMessage;
