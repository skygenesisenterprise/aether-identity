...
const totpService_1 = require("./totpService");
class AuthenticationService {
     ...
     * Create SSO session
     */
    async createSsoSession(userId, clientInfo) {
    ...
    async refreshToken(refreshToken, clientInfo) {
        try {
            ...
            const tokenData = tokenService_1.tokenService.verifyRefreshToken(refreshToken);
    ...
    async handleFailedLogin(user, clientInfo) {
        ...
        await this.logAuthEvent(clientInfo, 'LOGIN_FAILED', 'INVALID_CREDENTIALS', `Failed attempts: ${newFailedAttempts}, Locked: ${shouldLock}`);
    }
    async handleSuccessfulLogin(user, clientInfo) {
        await this.prisma.user.update({
            where: { id: user.id },
            data: {
    ...
    async createMfaSession(user, clientInfo) {
    ...
    async verifyMFA(user, code, sessionId) {
        if (user.mfaMethod === 'TOTP') {
            const result = await totpService_1.totpService.verifyTOTPToken(user.id, code);
            ...
            return await totpService_1.totpService.verifyBackupCode(user.id, code);
        }
        return false;
    }
    async generateTokens(user, clientId) {
        ...
        const idToken = tokenService_1.tokenService.generateIdToken(user, clientId || 'default');
        // Store refresh token
    ...
    async logAuthEvent(clientInfo, action, status, details) {
        if (!clientInfo)
            return;
        try {
