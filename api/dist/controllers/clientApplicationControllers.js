    ...
    constructor() {
        ...
        this.createClient = async (req, res) => {
            try {
                ...
                const client = await database_1.prisma.clientApplication.create({
                    data: {
                        id: (0, uuid_1.v4)(),
                        clientId,
                        name,
        ...
        this.listClients = async (req, res) => {
            try {
                ...
                const clients = await database_1.prisma.clientApplication.findMany({
        ...
        this.getClient = async (req, res) => {
            try {
                ...
                const client = await database_1.prisma.clientApplication.findFirst({
        ...
        this.updateClient = async (req, res) => {
            try {
                ...
                const { id } = req.params;
                const { name, description, redirectUris, allowedScopes, defaultScopes, skipConsent, logoUrl, homepageUrl, privacyPolicyUrl, tosUrl, isActive } = req.body;
                const userId = req.user?.id;
                if (!userId) {
                    res.status(401).json({
                        error: 'User authentication required',
                ...
                const existingClient = await database_1.prisma.clientApplication.findFirst({
                    where: { id, createdBy: userId }
                });
                if (!existingClient) {
                    res.status(404).json({
                ...
                // Update client
                const updatedClient = await database_1.prisma.clientApplication.update({
                    where: { id },
                    data: {
                        ...(name && { name }),
        ...
        this.deleteClient = async (req, res) => {
            try {
                ...
                // Delete client
                await database_1.prisma.clientApplication.delete({
                    where: { id }
                });
                res.status(200).json({
                    success: true,
         ...
         */
        this.rotateClientSecret = async (req, res) => {
            try {
                const { id } = req.params;
                const userId = req.user?.id;
                if (!userId) {
...
exports.ClientApplicationController = ClientApplicationController;
